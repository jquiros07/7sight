...
class ExtractAudioJobTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_it_marks_the_job_completed_with_the_output_location(): void
    {
        ...
        $processor->shouldReceive('extractAudio')->once()->with(Mockery::type('string'), Mockery::type('string'));

        (new ExtractAudioJob($job))->handle($processor);
    ...
    {
        $video = Video::factory()->create();
        $job = VideoToolJob::factory()->create(['video_id' => $video->id, 'type' => VideoToolType::AudioExtraction]);

        (new ExtractAudioJob($job))->failed(new Exception('ffmpeg exploded'));
